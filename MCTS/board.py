...
S_PLAYER1, S_PLAYER2, S_EMPTY, S_BLOCKED, = ['0', '1', '.', 'x'] #symbols
CHARTABLE = [(PLAYER1, S_PLAYER1), (PLAYER2, S_PLAYER2), (EMPTY, S_EMPTY), (BLOCKED, S_BLOCKED)]

# regular xy axes directions
DIRS = [
    ((-1, 0), "up"),
    ((0, 1), "right"),
    ((1, 0), "down"),
...
class Board:
    ...
    def legal_moves(self, my_id, players, pos=None):
        if pos is not None:
            row, col = pos
        else:
            my_player = players[my_id]
            row, col = my_player.row, my_player.col
        result = []
    ...
    def field_forward(self, move, p_id, pos):
        # curr_player = players[p_id]
        old_row, old_col = pos #curr_player.row, curr_player.col
        ((o_row, o_col),_) = move
        new_row, new_col = old_row + o_row, old_col + o_col
        self.cell[old_row][old_col] = [BLOCKED]
        self.cell[new_row][new_col] = [CHARTABLE[p_id][0]]
        # with open('../lightriders_mcts/tmp.txt', 'a') as f:
        #         print((old_row, old_col), (curr_player.row, curr_player.col), (new_row, new_col), (players[p_id].row, players[p_id].col), file=f)
        # players[p_id].row, players[p_id].col = new_row, new_col
        return new_row, new_col

    def field_reverse(self, move, p_id, pos):
        # curr_player = players[p_id]
        old_row, old_col = pos #curr_player.row, curr_player.col
        ((o_row, o_col),_) = move
        new_row, new_col = old_row - o_row, old_col - o_col
        ...
        #         print((o_row, o_col), (old_row, old_col), (curr_player.row, curr_player.col), (new_row, new_col), (players[p_id].row, players[p_id].col), file=f)
        # players[p_id].row, players[p_id].col = new_row, new_col

    def h1(self,my_id, pos):
        # my_player = players[my_id]
        # t_row = my_player.row 
        # t_col = my_player.col
        t_row, t_col = pos
    ...
    def h2(self,my_id, pos):
        ...
        while len(openlist) != 0:
            ...
            for (o_row, o_col), _ in DIRS:
                ...
                if self.is_legal(t_row, t_col, my_id):
                    if depth[t_row * self.width + t_col] == 400:     
